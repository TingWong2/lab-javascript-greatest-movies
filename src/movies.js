...
function getAllDirectors(arrayOfMovies) { 
  const arrayOfDirectors = arrayOfMovies.map((movie)=>{ return movie.director})
...
function howManyMovies(arrayOfMovies) {
  ...
  const filteredTable = arrayOfMovies.filter (movie => {
    //console.log(movie);
    return movie.genre.includes("Drama") && movie.director === "Steven Spielberg" 
  })
  console.log("mon result est : ", filteredTable.length);
...
function scoresAverage(arrayOfMovies) {
  ...
  const sum = arrayOfMovies.reduce((acc, currentValue) => {
     acc += currentValue.score;
     return acc;
  },0); 
  console.log("sum", sum);
  const average = sum / arrayOfMovies.length;
  console.log("score average",average);
  return Number(average.toFixed(2));
}

console.log(scoresAverage(movies));
console.log("******", movies.filter(movie => movie.score ===8).length);


// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(movies) {
  //1. filter tous les dramas movies de l'ensemble des movies
 
/* function dramaFilter (movie) { 
    const isDrama = movie.genre === "drama"
    return isDrama;
  }

  const dramaFilter = (movie) => movie.genre === "drama";
  */
  const dramaMovies = movies.filter(movie => movie.genre.includes("Drama"));
  console.log("$$$", movies.length, dramaMovies.length);

  //2.lire le score des movies dans dramamovies
  /*function readDramaScore ( movie ) { 
  return movie.score;
  }
  */
  const scoresTable = dramaMovies.map(movie  => movie.score)
  console.log("scoresTable",scoresTable);

  //3. faire la somme pour avoir la moyenne 
  ...
  const sumDramaMovies = scoresTable.reduce (reducer, 0 )
  console.log(sumDramaMovies);
  const dramaScoreAverage = sumDramaMovies / scoresTable.length; 
  console.log(dramaScoreAverage);
...
// 1. Extraire par titre
/*function readTitle ( movie ) { 
  return movie.title
}
  movie  =>  movie.title
*/
  
const titlesTable = movies.map( movie  =>  movie.title);

// 2. Trier par ordre alphabetique
  const sortedTitles = titlesTable.sort();
...
//1. retourner un nouveau tableau sans modifier l'original


const hoursToMinute= movies.map(function (movie){
  const clone = {...movie};  

  return {...movie, duration: movie.duration.split("",0) * 60 + split(3,4)}
});

//2. replaces the duration info of each of the movies for its equivalent in minutes.
